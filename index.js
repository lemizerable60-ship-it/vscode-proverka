window.addEventListener('load', async () => {
  try {
    ...
    class GeminiAIService {
        constructor() {
            const GoogleGenAI = window.genai?.GoogleGenAI;
            ...
            if (GoogleGenAI && apiKey) {
                try {
                 ...
                 if (!GoogleGenAI) {
                    console.warn("GoogleGenAI module not loaded. AI features are unavailable.");
                } else {
      ...
      return React.createElement('div', null,
        ...
        React.createElement('main', { className: "p-4 grid grid-cols-1 md:grid-cols-2 gap-4" },
          ...
          React.createElement(Card, { className: "cursor-pointer hover:shadow-lg transition-shadow", onClick: () => setScreen({ name: 'settings' }) },
            ...
            React.createElement('p', { className: "text-gray-600 dark:text-gray-400" }, "Внешний вид, импорт и экспорт")
          )
        )
      );
    };
                    ...
                    try {
                        ...
                        if (typeof text === 'string') {
                            ...
                            if (importedData.clients && importedData.testResults && importedData.reports) {
                                ...
                                // Import and apply theme settings
                                if(importedData.theme) {
                                    localStorage.setItem('theme', importedData.theme);
                                     if (importedData.theme === 'dark') document.documentElement.classList.add('dark');
                                     else document.documentElement.classList.remove('dark');
                                }
                                if(importedData.themeColor) {
                                    localStorage.setItem('themeColor', importedData.themeColor);
                                    document.documentElement.style.setProperty('--primary-color', importedData.themeColor);
                                }
                                alert('Данные и настройки успешно импортированы!');
                                setScreen({ name: 'home' });
                            } else {
            ...
            React.createElement('main', { className: "p-4 space-y-4" },
                 React.createElement(Card, null,
                    ...
                    React.createElement('div', {className: "mb-4"},
                      React.createElement('p', { className: "text-gray-600 dark:text-gray-400 mb-2" }, "Переключить тему оформления."),
                      React.createElement(Button, { onClick: toggleTheme, className:"flex items-center gap-2" }, React.createElement(ThemeIcon), "Сменить тему (Светлая/Темная)")
                    ),
                    React.createElement('div', null,
                      React.createElement('p', { className: "text-gray-600 dark:text-gray-400 mb-2" }, "Выбрать акцентный цвет."),
                       React.createElement('div', {className: "flex flex-wrap gap-2"},
                         THEME_COLORS.map(color => React.createElement('button', {
                           key: color.value,
                           onClick: () => changeThemeColor(color.value),
                           className: `w-10 h-10 rounded-full border-2 ${currentColor === color.value ? 'ring-2 ring-offset-2 ring-primary-500' : 'border-transparent'}`,
                ...
                React.createElement(Card, null,
                    ...
                    React.createElement('p', { className: "text-gray-600 dark:text-gray-400 mb-4" }, "Сохранить всю базу, включая настройки темы, в один JSON-файл."),
                ...
                React.createElement(Card, null,
                    ...
                    React.createElement('p', { className: "text-gray-600 dark:text-gray-400 mb-4" }, "Загрузить данные из файла. Внимание: это перезапишет все текущие данные!"),
    ...
    if (rootDiv) {
      rootDiv.innerHTML = `<div style="padding: 20px; color: red; background-color: #fff;">
        <h1>Application Error</h1>
        <p>An error occurred during initialization. Please check the console for details.</p>
        <pre>${error.stack}</pre>
      </div>`;
    }
  }
